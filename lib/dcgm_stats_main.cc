    ...
    for (const auto &gpuPair : field_val_map)
    {
        ...
        for (const auto &fieldValue : fieldValues)
        {
            ...
            case DCGM_FT_DOUBLE:
                ...
                valueInterpretation = 1;
                std::cout << "here" << std::endl;
                std::cout << valueInterpretation << std::endl;

                break;
            case DCGM_FT_INT64:
                value = fieldValue.value.i64;
                valueInterpretation = 2;
                std::cout << valueInterpretation << std::endl;
                break;
            ...
            std::cout << "Data:" << gpuId << ',' << fieldName << ',' << fieldValue.fieldId << ',' << value << ',' << valueInterpretation << std::endl;
