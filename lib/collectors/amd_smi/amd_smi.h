...
// Outcome of a single AMD SMI read. The collector latches a metric off only on
// Unsupported (permanent: the firmware/driver reports NOT_SUPPORTED or
// NOT_YET_IMPLEMENTED); Error is transient and retried on the next tick.
...
class AmdSmi
{
   public:
    ...
    ReadStatus ReadMemory(uint32_t gpu_id, AmdSmiMemory& out) noexcept;

    // PCIe traffic over a 1-second active measurement performed by the
    // amdgpu kernel driver. Returns Unsupported on ASICs whose driver
    ...
    // exposed.
    ReadStatus ReadPcieThroughput(uint32_t gpu_id, AmdSmiThroughput& out) noexcept;
    ...
    ReadStatus ReadMetrics(uint32_t gpu_id, amdsmi_gpu_metrics_t& out) noexcept;
