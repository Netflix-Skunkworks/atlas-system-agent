   ...
   public:
    ...
    std::vector<MountPoint> filter_interesting_mount_points(const std::vector<MountPoint>& mount_points) const noexcept
    {
        ...
        std::sort(v.begin(), v.end(),
                  [](const MountPoint& a, const MountPoint& b) { return a.mount_point < b.mount_point; });
        return v;
    }
...
TEST(Disk, MountPoints)
{
    ...
    EXPECT_EQ(fmt::format("{}", mount_points.at(10)),
              "MP{dev#=7:3,mp=/snap/amazon-ssm-agent/2012,dev=/dev/loop3,type=squashfs}");
    ...
    EXPECT_EQ(fmt::format("{}", mount_points.at(14)),
              "MP{dev#=7:7,mp=/snap/amazon-ssm-agent/4046,dev=/dev/loop7,type=squashfs}");
...
TEST(Disk, UpdateDiskStats)
{
    ...
    std::unordered_set<std::string> values = {"C:disk.io.bytes,dev=xvda,id=read:445441024.000000\n",
