    ...
    const std::vector<MountPoint>& mount_points) const noexcept
{
    ...
    for (const auto& mp : mount_points)
    {
        ...
        if (starts_with(mp.mount_point.c_str(), "/dev") || starts_with(mp.mount_point.c_str(), "/mnt/docker") ||
            starts_with(mp.mount_point.c_str(), "/mnt/jenkins") ||
            starts_with(mp.mount_point.c_str(), "/mnt/kubelet") || starts_with(mp.mount_point.c_str(), "/proc") ||
            starts_with(mp.mount_point.c_str(), "/run") || starts_with(mp.mount_point.c_str(), "/sys") ||
            starts_with(mp.mount_point.c_str(), "/tmp/buildkit") || starts_with(mp.mount_point.c_str(), "/var/lib"))
        {
            continue;
        }
...
void Disk::stats_for_interesting_mps(std::function<void(Disk*, const MountPoint&)> stats_fn) noexcept
{
...
void Disk::disk_stats() noexcept { do_disk_stats(absl::Now()); }
...
void Disk::diskio_stats(absl::Time start) noexcept
{
    ...
    for (const auto& st : stats)
    {
        ...
        auto readTags = std::unordered_map<std::string, std::string>{{"id", kRead}, {"dev", st.device}};
        auto writeTags = std::unordered_map<std::string, std::string>{{"id", kWrite}, {"dev", st.device}};
        ...
        {
            continue;  // ignore multiple devices for disk.io.ops and disk.percentBusy - they do not provide timing
                       // stats
        }
