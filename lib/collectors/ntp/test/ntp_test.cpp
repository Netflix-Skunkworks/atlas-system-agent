...
double get_default_sample_age(const NtpTest& ntp) { return absl::ToDoubleSeconds(TestClock::now() - ntp.lastSample()); }
...
TEST(Ntp, Stats)
{
    ...
    std::vector<std::string> sources = {"^,*,169.254.169.123,3,8,377,74,-0.000027989,-0.000076710,0.000319246\n",
...
TEST(Ntp, StatsInvalid)
{
    ...
    std::string tracking = "A9FEA97B,1.2.3.4,4,1.1,foo,-0.021,1,-2,-0.022,0.079,0.0005,0.0001,775.8,Normal\n";

    std::vector<std::string> sources = {"^,*,1.2.3.4,3,8,377,abc,-0.000027989,-0.000076710,0.000319246\n",
                                        "^,-,10.229.0.50,2,10,340,7219,0.002353442,0.001586549,0.049785987\n",
                                        "^,-,172.16.1.2,2,10,337,1028,0.000021583,-0.000021316,0.049278442\n"};
...
TEST(Ntp, NoSources)
{
    ...
    std::string tracking = "A9FEA97B,1.2.3.4,4,1.1,10,-0.021,1,-2,-0.022,0.079,0.0005,0.0001,775.8,Normal\n";

    std::vector<std::string> sources = {"^,*,1.2.3.5,3,8,377,abc,-0.000027989,-0.000076710,0.000319246\n",
                                        "^,-,10.229.0.50,2,10,340,7219,0.002353442,0.001586549,0.049785987\n",
                                        "^,-,172.16.1.2,2,10,337,1028,0.000021583,-0.000021316,0.049278442\n"};
    ntp.stats(tracking, sources);
    auto memoryWriter = static_cast<MemoryWriter*>(WriterTestHelper::GetImpl());
