...
static void copy_entry(const struct btrfs_qgroup* in, struct btrfs_qgroup_info* out) {
  out->qgroupid = in->qgroupid;
  out->generation = in->generation;
	out->rfer = in->rfer;
	out->rfer_cmpr = in->rfer_cmpr;
	out->excl = in->excl;
	out->excl_cmpr = in->excl_cmpr;
	out->flags = in->flags;
	out->max_rfer = in->max_rfer;
	out->max_excl = in->max_excl;
	out->rsv_rfer = in->rsv_rfer;
	out->rsv_excl = in->rsv_excl;
}

static void get_all_qgroups(struct qgroup_lookup* qgroup_lookup, 
  struct btrfs_qgroup_info* out, 
  size_t* size)
{

	struct rb_node* n = rb_first(&qgroup_lookup->root);
  size_t i = 0;
	while (n) {
		struct btrfs_qgroup* entry = rb_entry(n, struct btrfs_qgroup, sort_node);
    struct btrfs_qgroup_info* this_entry = &out[i];
    copy_entry(entry, this_entry);
    i++;
		n = rb_next(n);
    if (i >= *size) {
      break;
           ...
           size_t* size)
{
	...
	__filter_and_sort_qgroups(&qgroup_lookup, &sort_tree,
				  filter_set, comp_set);
	get_all_qgroups(&sort_tree, out, size);
